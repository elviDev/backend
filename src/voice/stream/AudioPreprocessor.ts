...
export interface PreprocessorConfig {
  ...
  agcEnabled: boolean; // Automatic Gain Control
  targetRMS: number; // Target RMS level for normalization
  noiseGateThreshold: number; // Noise gate threshold
  highPassFreq: number; // High pass filter frequency
  lowPassFreq: number; // Low pass filter frequency
  sampleRate: number;
}
...
export class AudioPreprocessor {
  ...
  private runningRMS: number = 0;
  private noiseProfile: Float32Array | null = null;
  private agcGain: number = 1.0;
  private dcOffset: number = 0;

  // Filter state (simple IIR filters)
  private highPassState: { x1: number; x2: number; y1: number; y2: number } = {
    x1: 0,
    x2: 0,
    y1: 0,
    y2: 0,
  };
  private lowPassState: { x1: number; x2: number; y1: number; y2: number } = {
    x1: 0,
    x2: 0,
    y1: 0,
    y2: 0,
  ...
  constructor(config: Partial<PreprocessorConfig> = {}) {
    ...
    logger.debug('Audio preprocessor initialized', { config: this.config });
  }
  ...
  async process(audioBuffer: Buffer): Promise<Buffer> {
    ...
    try {
      ...
      let processedSamples: Float32Array = new Float32Array(samples.length);
      processedSamples.set(samples);
      const appliedProcessing: string[] = [];

      // Step 1: DC offset removal
      processedSamples = this.removeDCOffset(processedSamples) as Float32Array;
      appliedProcessing.push('dc_removal');

      // Step 2: High-pass filter (remove low frequency noise)
      if (this.config.highPassFilter) {
        processedSamples = this.applyHighPassFilter(processedSamples) as Float32Array;
        appliedProcessing.push('high_pass_filter');
      }

      // Step 3: Low-pass filter (anti-aliasing)
      if (this.config.lowPassFilter) {
        processedSamples = this.applyLowPassFilter(processedSamples) as Float32Array;
        appliedProcessing.push('low_pass_filter');
      }

      // Step 4: Noise gate
      processedSamples = this.applyNoiseGate(processedSamples) as Float32Array;
      ...
      if (this.config.noiseReduction) {
        ...
        processedSamples = result.samples as Float32Array;
      ...
      if (this.config.agcEnabled) {
        processedSamples = this.applyAutomaticGainControl(processedSamples) as Float32Array;
      ...
      if (processingTime > 50) {
        logger.warn('Slow audio preprocessing', {
   ...
   */
  async processDetailed(audioBuffer: Buffer): Promise<ProcessingResult> {
    const startTime = performance.now();
  ...
  private applyNoiseReduction(samples: Float32Array): {
  ...
  private applyAutomaticGainControl(samples: Float32Array): Float32Array {
    const output = new Float32Array(samples.length);
  ...
  private normalizeVolume(samples: Float32Array): {
    samples: Float32Array;
    clippingPrevented: boolean;
  } {
    const output = new Float32Array(samples.length);
  ...
  private checkForClipping(samples: Float32Array): boolean {
    for (let i = 0; i < samples.length; i++) {
