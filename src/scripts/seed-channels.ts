...
import ChannelRepository from '../db/ChannelRepository';
...
export async function seedChannels() {
  try {
    ...
    logger.info('Database connection test:', testResult.rows[0]);
    
    // Check if any channels exist
    const existingChannels = await query('SELECT COUNT(*) as count FROM channels WHERE deleted_at IS NULL');
    const channelCount = parseInt(existingChannels.rows[0].count, 10);
    
    if (channelCount > 0) {
      logger.info(`Found ${channelCount} existing channels, skipping seed`);
      return;
    }

    // Get users to assign as channel owners and members
    const usersResult = await query('SELECT id, email, role FROM users WHERE deleted_at IS NULL ORDER BY created_at ASC LIMIT 10');
    const users = usersResult.rows;
    
    if (users.length === 0) {
      logger.warn('No users found, cannot seed channels without users');
      return;
    }
    ...
    try {
      const categoriesResult = await query('SELECT COUNT(*) as count FROM categories WHERE deleted_at IS NULL');
      ...
      if (categoryCount === 0) {
        ...
        for (const category of defaultCategories) {
          await query(
            'INSERT INTO categories (name, description, metadata) VALUES ($1, $2, $3)',
            [category.name, category.description, JSON.stringify({ color: category.color })]
          );
        }
        
        logger.info(`Created ${defaultCategories.length} categories`);
      }
    } catch (error) {
      logger.warn('Categories table may not exist, skipping category creation');
    }
    ...
    for (const channelData of seedChannels) {
      try {
        ...
        await query(
          `UPDATE channels SET activity_stats = $1 WHERE id = $2`,
          [
            JSON.stringify({
    ...
    logger.info(`Channel seeding completed: ${createdCount}/${seedChannels.length} channels created`);
  } catch (error) {
    logger.error('Channel seeding failed:', error);
...
if (require.main === module) {
  seedChannels()
    .then(() => {
      logger.info('Channel seeding script completed successfully');
      process.exit(0);
    })
    .catch((error) => {
      logger.error('Channel seeding script failed:', error);
      process.exit(1);
    });
}