...
async function verifyData() {
  try {
    ...
    // Users
    const usersResult = await query('SELECT COUNT(*) as count, role FROM users WHERE deleted_at IS NULL GROUP BY role ORDER BY role');
    console.log('\n👥 USERS:');
    let totalUsers = 0;
    ...
    // User Details
    const userDetails = await query('SELECT name, email, role FROM users WHERE deleted_at IS NULL ORDER BY role DESC, name');
    console.log('\n👤 USER ACCOUNTS:');
    ...
    // Channels
    const channelsResult = await query('SELECT COUNT(*) as count, channel_type FROM channels WHERE deleted_at IS NULL GROUP BY channel_type ORDER BY channel_type');
    console.log('\n📂 CHANNELS:');
    let totalChannels = 0;
    ...
    // Channel Details
    const channelDetails = await query('SELECT name, channel_type, privacy_level FROM channels WHERE deleted_at IS NULL ORDER BY channel_type, name');
    console.log('\n📋 CHANNEL LIST:');
    ...
    // Messages
    const messagesResult = await query('SELECT COUNT(*) as count FROM messages WHERE deleted_at IS NULL');
    const threadRepliesResult = await query('SELECT COUNT(*) as count FROM messages WHERE thread_root_id IS NOT NULL AND deleted_at IS NULL');
    const directRepliesResult = await query('SELECT COUNT(*) as count FROM messages WHERE reply_to_id IS NOT NULL AND thread_root_id IS NULL AND deleted_at IS NULL');
    const originalMessagesResult = await query('SELECT COUNT(*) as count FROM messages WHERE reply_to_id IS NULL AND deleted_at IS NULL');
    ...
    // Tasks
    const tasksResult = await query('SELECT COUNT(*) as count FROM tasks WHERE deleted_at IS NULL');
    const taskCommentsResult = await query('SELECT COUNT(*) as count FROM task_comments WHERE deleted_at IS NULL');
    ...
    const tasksByChannel = await query(`
    ...
    // Reactions
    const reactionsResult = await query('SELECT COUNT(*) as count FROM message_reactions');
