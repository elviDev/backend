...
async function addStatusBasedComments() {
  try {
    logger.info('🔄 Adding status-based comments to non-pending tasks...');
    ...
    logger.info(`📊 Found ${nonPendingTasks.length} non-pending tasks out of ${tasks.length} total`);
    ...
    for (const task of nonPendingTasks) {
      try {
        logger.info(`💼 Processing ${task.status} task: "${task.title}"`);
        ...
        const statusComments = statusBasedComments[task.status as keyof typeof statusBasedComments] || [];
        if (statusComments.length === 0) {
          logger.warn(`⚠️ No status comments defined for status: ${task.status}`);
          continue;
        }
        ...
        for (let i = 0; i < numComments; i++) {
          ...
          if (usedComments.has(commentText)) break; // No more unique comments available
          ...
          commentsAdded++;
          logger.debug(`  ✅ Comment added by ${commenter!.name}: "${commentText.substring(0, 50)}..."`);
          ...
          if ((task.status === 'completed' || task.status === 'review') && Math.random() > 0.6) {
            ...
            if (replier && progressReplies.length > 0) {
              ...
              commentsAdded++;
              logger.debug(`    ↳ Reply by ${replier.name}: "${replyText}"`);
            }
          }
          ...
          if (Math.random() < reactionChance) {
            try {
              ...
              logger.debug(`Failed to add reaction: ${reactionError}`);
            }
          }
        }

        tasksProcessed++;
        logger.info(`  ✅ Added ${numComments} comments to "${task.title}" (${task.status})`);
    ...
    logger.info(`  - Task statuses covered: ${Object.keys(statusBasedComments).join(', ')}`);
    ...
    logger.info(`📈 Non-pending task breakdown:`);
    ...
    logger.error('❌ Failed to add status-based comments:', error);
    throw error;
  }
}
