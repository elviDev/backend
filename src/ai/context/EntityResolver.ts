...
export class EntityResolver {
  ...
  private fuzzyCache: Map<string, any[]> = new Map();
  
  constructor(db: DatabaseManager) {
    this.db = db;
    
    logger.debug('Entity Resolver initialized');
  }
  ...
  async resolveUser(name: string, context: ContextData): Promise<ResolvedUser | null> {
    ...
    try {
      ...
      logger.debug('User resolution failed', {
        name,
      ...
      logger.error('User resolution error', {
        name,
  ...
  async resolveChannel(name: string, context: ContextData): Promise<ResolvedChannel | null> {
    ...
    try {
      ...
      logger.debug('Channel resolution failed', {
        name,
        resolutionTime: `${resolutionTime.toFixed(2)}ms`
      });
   ...
   */
  async resolveTask(reference: string, context: ContextData): Promise<ResolvedTask | null> {
    const startTime = performance.now();
    ...
    try {
      ...
      logger.error('Task resolution error', {
        reference,
        error: (error as any).message,
        resolutionTime: `${resolutionTime.toFixed(2)}ms`
      });
   ...
   */
  async validateAndEnhanceEntities(
    entities: any, 
    context: ContextData
  ...
  private async findUserExactMatch(name: string, context: ContextData): Promise<ResolvedUser | null> {
    // Check team members first (most common case)
    const exactMatch = context.teamMembers.find(member => 
  ...
  private async findUserFuzzyMatch(name: string, context: ContextData): Promise<ResolvedUser | null> {
  ...
  private async resolveUserByRole(name: string, context: ContextData): Promise<ResolvedUser | null> {
    ...
    for (const [role, keywords] of Object.entries(roleKeywords)) {
      if (keywords.some(keyword => nameLower.includes(keyword))) {
        const roleMatch = context.teamMembers.find(member => 
  ...
  private findChannelExactMatch(name: string, context: ContextData): ResolvedChannel | null {
    const exactMatch = context.activeChannels.find(channel => 
  ...
  private findChannelFuzzyMatch(name: string, context: ContextData): ResolvedChannel | null {
  ...
  private async findChannelInDatabase(name: string, context: ContextData): Promise<ResolvedChannel | null> {
    try {
      ...
      logger.error('Database channel search failed', {
        name,
        error: (error as any).message
      });
    }
  ...
  private findTaskExactMatch(reference: string, context: ContextData): ResolvedTask | null {
    const exactMatch = context.recentTasks.find(task => 
      task.title.toLowerCase() === reference.toLowerCase()
    );
  ...
  private findTaskFuzzyMatch(reference: string, context: ContextData): ResolvedTask | null {
  ...
  private async resolveTaskContextually(reference: string, context: ContextData): Promise<ResolvedTask | null> {
    ...
    if (refLower.includes('urgent') || refLower.includes('priority')) {
      // Find urgent tasks
      const urgentTask = context.recentTasks.find(task => 
        task.dueDate && new Date(task.dueDate) <= new Date(Date.now() + 86400000) // Due within 24 hours
      );
  ...
  private fuzzyMatchUsers(query: string, users: ContextData['teamMembers']): Array<{user: any, score: number}> {
    return users
      .map(user => ({
        user,
  ...
  private fuzzyMatchChannels(query: string, channels: ContextData['activeChannels']): Array<{channel: any, score: number}> {
    return channels
      .map(channel => ({
        channel,
  ...
  private fuzzyMatchTasks(query: string, tasks: ContextData['recentTasks']): Array<{task: any, score: number}> {
    return tasks
      .map(task => ({
        task,
