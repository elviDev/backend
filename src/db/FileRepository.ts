...
class FileRepository extends BaseRepository<FileEntity> {
   ...
   */
  async createFile(fileData: CreateFileData, client?: DatabaseClient): Promise<FileEntity> {
    ...
    const fileToCreate: Partial<Omit<FileEntity, keyof BaseEntity>> = {
  ...
  async findChannelFiles(
    ...
    const result = await this.executeRawQuery<FileWithUploader>(sql, params, client);
    return result.rows;
  }
  ...
  async findTaskFiles(
    ...
    const result = await this.executeRawQuery<FileWithUploader>(
      sql,
      [taskId, limit, offset],
      client
    );
   ...
   */
  async recordFileAccess(fileId: string, client?: DatabaseClient): Promise<void> {
    const sql = `
    ...
    try {
      ...
      logger.error({ error, fileId }, 'Failed to record file access');
    }
  }
   ...
   */
  async recordFileDownload(fileId: string, client?: DatabaseClient): Promise<void> {
    const sql = `
      UPDATE ${this.tableName}
    ...
    try {
      ...
      logger.error({ error, fileId }, 'Failed to record file download');
    }
  }
  ...
  async updateVirusScanStatus(
    fileId: string,
    status: FileEntity['virus_scan_status'],
    ...
    try {
      const queryResult = await this.executeRawQuery(
        sql,
        [fileId, status, JSON.stringify(result || {})],
        client
      );
      ...
      logger.error({ error, fileId, status }, 'Failed to update virus scan status');
      return false;
    }
  }
   ...
   * Get file statistics
   */
  async getFileStats(client?: DatabaseClient): Promise<{
    totalFiles: number;
    totalSize: number;
    ...
    const [statsResult, topFilesResult] = await Promise.all([
      ...
      this.executeRawQuery<any>(topFilesSql, [], client),
    ]);
    ...
    return {
      ...
      mostDownloadedFiles: topFilesResult.rows.map((row: any) => ({
        id: row.id,
  ...
  async cleanupExpiredFiles(client?: DatabaseClient): Promise<number> {
    ...
    try {
      ...
      logger.error({ error }, 'Failed to clean up expired files');
      return 0;
    }
  }
