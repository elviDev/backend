...
class UserRepository extends BaseRepository<User> {
   ...
   */
  async createUser(userData: CreateUserData, client?: DatabaseClient): Promise<User> {
    ...
    if (userData.role === 'ceo') {
      ...
      if (existingCEO.length > 0) {
        throw new ConflictError('CEO role already exists', { role: 'ceo' });
      }
    }

    // Hash password
    const password_hash = await bcrypt.hash(userData.password, config.security.bcryptRounds);
  ...
  async findByEmail(
    ...
    const result = await this.executeRawQuery<User>(sql, [email], client);
  ...
  async findByEmailForAuth(
    ...
    const result = await this.executeRawQuery<User & { password_hash: string }>(
      sql,
      [email],
      client
    );
   ...
   */
  async recordSuccessfulLogin(userId: string, client?: DatabaseClient): Promise<void> {
    const sql = `
      UPDATE ${this.tableName}
      SET 
        failed_login_attempts = 0,
   ...
   */
  async incrementFailedLoginAttempts(userId: string, client?: DatabaseClient): Promise<void> {
    ...
    logger.warn({ userId }, 'Failed login attempt recorded');
  }

  /**
   * Update last active timestamp
   */
  async updateLastActive(userId: string, client?: DatabaseClient): Promise<void> {
    const sql = `
      UPDATE ${this.tableName}
      SET last_active = NOW()
      WHERE id = $1 AND deleted_at IS NULL
   ...
   */
  async verifyPasswordResetToken(token: string, client?: DatabaseClient): Promise<User | null> {
    const sql = `
      SELECT ${this.selectFields.join(', ')}
      FROM ${this.tableName}
    ...
    const result = await this.executeRawQuery<User>(sql, [token], client);
    return result.rows[0] || null;
  }
   ...
   */
  async verifyEmail(token: string, client?: DatabaseClient): Promise<User | null> {
    const sql = `
      UPDATE ${this.tableName}
      SET 
        email_verified = true,
    ...
    if (result.rows.length > 0 && result.rows[0]) {
      logger.info({ userId: result.rows[0].id }, 'Email verified successfully');
    }
   ...
   */
  async incrementOTPAttempts(userId: string, client?: DatabaseClient): Promise<void> {
    const sql = `
      UPDATE ${this.tableName}
   ...
   */
  async verifyEmailWithOTP(userId: string, client?: DatabaseClient): Promise<User | null> {
    const sql = `
      UPDATE ${this.tableName}
      SET 
        email_verified = true,
    ...
    const result = await this.executeRawQuery<User>(sql, [userId], client);

    if (result.rows.length > 0 && result.rows[0]) {
      logger.info({ userId: result.rows[0].id }, 'Email verified successfully with OTP');
    }
  ...
  async findByEmailForOTP(
    ...
    const result = await this.executeRawQuery<User & { 
      email_verification_otp: string | null;
      email_verification_otp_expires: Date | null;
      otp_attempts: number;
    }>(sql, [email], client);
  ...
  async searchUsers(
    ...
    const result = await this.executeRawQuery<User>(sql, [searchPattern, limit, offset], client);
    return result.rows;
  }
   ...
   */
  async getActiveUsers(withinHours: number = 24, client?: DatabaseClient): Promise<User[]> {
    const sql = `
      SELECT ${this.selectFields.join(', ')}
      FROM ${this.tableName}
    ...
    const result = await this.executeRawQuery<User>(sql, [], client);
    return result.rows;
  }
   ...
   * Get user statistics
   */
  async getUserStats(client?: DatabaseClient): Promise<UserStats> {
    const sql = `
      SELECT 
    ...
    const result = await this.executeRawQuery<any>(sql, [], client);
   ...
   * Unlock user account
   */
  async unlockAccount(userId: string, client?: DatabaseClient): Promise<boolean> {
    const sql = `
      UPDATE ${this.tableName}
      SET 
        failed_login_attempts = 0,
    ...
    if (result.rowCount > 0) {
      logger.info({ userId }, 'User account unlocked');
    }
   ...
   */
  async getUsersForChannelAssignment(
    channelId: string,
    ...
    const result = await this.executeRawQuery<User>(sql, params, client);
    return result.rows;
  }
   ...
   */
  async findUnverifiedUsers(client?: DatabaseClient): Promise<User[]> {
    const sql = `
      SELECT ${this.selectFields.join(', ')}
      FROM ${this.tableName}
   ...
   */
  async deleteUnverifiedUsers(client?: DatabaseClient): Promise<number> {
    const sql = `
    ...
    logger.info({ deleteCount: result.rowCount }, 'Deleted unverified users');
