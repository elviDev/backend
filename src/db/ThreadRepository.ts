...
class ThreadRepository extends BaseRepository<ThreadStatistics> {
  constructor() {
  ...
  async createThread(
    ...
    const messageResult = await this.executeRawQuery(
      updateMessageSql,
      [messageId],
      client
    );
    ...
    const result = await this.executeRawQuery<ThreadStatistics>(
      threadStatsSql,
      [messageId],
      client
    );
  ...
  async getThreadWithDetails(
    ...
    const result = await this.executeRawQuery<ThreadWithDetails>(
      sql,
      [threadRootId],
      client
    );
  ...
  async getThreadReplies(
    ...
    const countResult = await this.executeRawQuery<{ total: string }>(
      countSql,
      [threadRootId],
      client
    );
    ...
    const repliesResult = await this.executeRawQuery<ThreadReply>(
      repliesSql,
  ...
  async addThreadReply(
    ...
    const messageResult = await this.executeRawQuery<{ id: string }>(
      insertSql,
      [
        threadRootId,
        replyData.user_id,
    ...
    const replyResult = await this.executeRawQuery<ThreadReply>(
      getReplySQL,
      [replyId],
      client
    );
  ...
  async getChannelThreads(
    ...
    const threadsResult = await this.executeRawQuery<{ id: string }>(
      threadsSql,
