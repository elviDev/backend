...
exports.FileCommandParser = exports.FileOperation = void 0;
...
(function (FileOperation) {
    ...
    FileOperation["ORGANIZE"] = "organize";
...
class FileCommandParser {
    performanceMetrics = [];
    constructor() {
        logger_1.logger.info('File Command Parser initialized');
    }
    /**
     * Parse file-related voice command
     * Target: <500ms parsing time
     */
    async parseFileCommand(transcript, context, userContext) {
        const startTime = perf_hooks_1.performance.now();
        const commandId = `file_cmd_${Date.now()}_${Math.random().toString(36).substring(7)}`;
        try {
            logger_1.logger.debug('Parsing file command', {
                commandId,
            ...
            logger_1.logger.info('File command parsed successfully', {
                commandId,
                operation,
                fileName: parameters.fileName,
            ...
            const processingTime = perf_hooks_1.performance.now() - startTime;
            this.recordPerformance(processingTime);
            logger_1.logger.error('File command parsing failed', {
                commandId,
                error: error.message,
                transcript: transcript.substring(0, 100),
                processingTime: `${processingTime.toFixed(2)}ms`,
     ...
     */
    extractFileParameters(transcript) {
     ...
     */
    async resolveFileTargets(targetEntities, context) {
        const resolved = {
            channels: [],
            tasks: [],
            users: [],
     ...
     */
    identifyFileOperation(transcript) {
        const lowerTranscript = transcript.toLowerCase();
        // Upload patterns
     ...
     */
    extractTargetEntities(transcript) {
        const entities = [];
        // Channel patterns
     ...
     */
    inferFileType(transcript, fileName) {
        if (fileName) {
     ...
     */
    generateContextualTags(transcript) {
        const tags = [];
        const lowerTranscript = transcript.toLowerCase();
     ...
     */
    findBestChannelMatch(name, channels) {
        let bestMatch = null;
        let bestScore = 0;
     ...
     */
    calculateMatchConfidence(input, target) {
        ...
        const commonWords = inputWords.filter((word) => targetWords.some((targetWord) => targetWord.includes(word) || word.includes(targetWord)));
     ...
     */
    calculateConfidence(transcript, operation, resolvedTargets) {
        ...
        const hasOperationKeyword = keywords.some((keyword) => transcript.toLowerCase().includes(keyword));
        if (hasOperationKeyword) {
            confidence += 0.2;
        }
        // Entity resolution confidence
        const totalTargets = resolvedTargets.channels.length + resolvedTargets.tasks.length + resolvedTargets.users.length;
        if (totalTargets > 0) {
            const avgEntityConfidence = [...resolvedTargets.channels, ...resolvedTargets.tasks, ...resolvedTargets.users].reduce((sum, entity) => sum + entity.confidence, 0) / totalTargets;
...
exports.FileCommandParser = FileCommandParser;
//# sourceMappingURL=FileCommandParser.js.map