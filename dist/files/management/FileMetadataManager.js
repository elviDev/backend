...
exports.FileMetadataManager = exports.FileStatus = void 0;
...
class FileMetadataManager {
    ...
    performanceMetrics = [];
    constructor() {
        this.db = db_1.DatabaseManager.getInstance();
        logger_1.logger.info('File Metadata Manager initialized');
    }
    /**
     * Create file record before upload process
     * Target: <200ms creation time
     */
    async createFileRecord(name, originalName, size, contentType, s3Key, uploadedBy, organizationId, description, tags = []) {
      ...
      `, [
            ...
            const processingTime = perf_hooks_1.performance.now() - startTime;
            this.recordPerformance(processingTime);
            logger_1.logger.error('Failed to create file record', {
                error: error.message,
                name,
                uploadedBy,
                organizationId,
     ...
     */
    async updateFileStatus(fileId, status, error) {
        const startTime = perf_hooks_1.performance.now();
        try {
            let query = `
        UPDATE files 
      ...
      `;
            ...
            if (result.rowCount === 0) {
                logger_1.logger.warn('File not found for status update', { fileId, status });
                return false;
            }
            const processingTime = perf_hooks_1.performance.now() - startTime;
            logger_1.logger.debug('File status updated', {
                fileId,
                status,
                processingTime: `${processingTime.toFixed(2)}ms`,
            ...
            logger_1.logger.error('Failed to update file status', {
     ...
     */
    async linkFileToEntity(fileId, entityType, entityId, linkType = 'attachment', linkedBy) {
        const startTime = perf_hooks_1.performance.now();
        try {
            await this.db.query(`
      ...
      `, [fileId, entityType, entityId, linkType, linkedBy]);
            const processingTime = perf_hooks_1.performance.now() - startTime;
            logger_1.logger.info('File linked to entity', {
                fileId,
                entityType,
                entityId,
                linkType,
            ...
            logger_1.logger.error('Failed to link file to entity', {
                error: error.message,
                fileId,
                entityType,
                entityId,
     ...
     */
    async unlinkFileFromEntity(fileId, entityType, entityId, linkType) {
        try {
            let query = `
      ...
      `;
            ...
            const result = await this.db.query(query, params);
            logger_1.logger.info('File unlinked from entity', {
                fileId,
                entityType,
                entityId,
                linkType,
            ...
            logger_1.logger.error('Failed to unlink file from entity', {
                error: error.message,
                fileId,
                entityType,
                entityId
    ...
    async getFileRecord(fileId) {
      ...
      `, [fileId, FileStatus.DELETED]);
            ...
            logger_1.logger.error('Failed to get file record', {
                error: error.message,
                fileId
            });
            return null;
    ...
    async searchFiles(query) {
        ...
        try {
            ...
            const conditions = ['f.organization_id = $1', 'f.status != $2'];
            const params = [query.organizationId, FileStatus.DELETED];
            let paramIndex = 3;
            if (query.userId) {
      ...
      `;
            ...
            logger_1.logger.error('File search failed', {
                error: error.message,
                query
            });
            return {
     ...
     */
    async getFilesForEntity(entityType, entityId, linkType) {
        try {
            let query = `
        SELECT 
          f.id, f.name, f.original_name as "originalName", f.size, 
      ...
      `;
            ...
            logger_1.logger.error('Failed to get files for entity', {
                error: error.message,
                entityType,
                entityId,
                linkType
    ...
    async recordDownload(fileId) {
      ...
      `, [fileId]);
            ...
            logger_1.logger.warn('Failed to record download', {
                error: error.message,
                fileId
            });
        }
    ...
    async deleteFileRecord(fileId) {
      ...
      `, [fileId, FileStatus.DELETED]);
        ...
        `, [fileId]);
                logger_1.logger.info('File record deleted', { fileId });
                return true;
            }
            return false;
        }
        catch (error) {
            logger_1.logger.error('Failed to delete file record', {
                error: error.message,
                fileId
            });
            return false;
...
//# sourceMappingURL=FileMetadataManager.js.map