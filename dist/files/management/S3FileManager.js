...
class S3FileManager {
    ...
    constructor(config) {
        ...
        logger_1.logger.info('S3 File Manager initialized', {
            region: this.config.region,
     ...
     */
    async generatePresignedUploadUrl(fileName, contentType, fileSize, metadata) {
        ...
        try {
            ...
            const uploadUrl = await (0, s3_request_presigner_1.getSignedUrl)(this.client, putCommand, {
            ...
            if (processingTime > 2000) {
                logger_1.logger.warn('Presigned URL generation exceeded 2 second target', {
            ...
            logger_1.logger.info('Presigned upload URL generated successfully', {
                fileId,
                fileName,
            ...
            const processingTime = perf_hooks_1.performance.now() - startTime;
            this.recordPerformance(processingTime);
            logger_1.logger.error('Failed to generate presigned upload URL', {
                error: error.message,
                fileName,
                fileSize,
                processingTime: `${processingTime.toFixed(2)}ms`
    ...
    async generatePresignedDownloadUrl(key, expiresIn = 3600) {
        ...
        try {
            ...
            const downloadUrl = await (0, s3_request_presigner_1.getSignedUrl)(this.client, getCommand, {
                expiresIn
            });
            const processingTime = perf_hooks_1.performance.now() - startTime;
            logger_1.logger.debug('Presigned download URL generated', {
                key,
                processingTime: `${processingTime.toFixed(2)}ms`,
            ...
            logger_1.logger.error('Failed to generate presigned download URL', {
                error: error.message,
                key
            });
    ...
    async getFileMetadata(key) {
        try {
            ...
            logger_1.logger.error('Failed to get file metadata', {
                error: error.message,
                key
            });
            return null;
    ...
    async getHealthStatus() {
        try {
            ...
            logger_1.logger.error('S3 health check failed', { error: error.message });
            return {
                status: 'unhealthy',
                details: {
                    error: error.message,
    ...
    generateFileKey(fileId, originalName, userId, organizationId) {
