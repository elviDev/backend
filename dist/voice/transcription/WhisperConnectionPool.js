...
exports.WhisperConnectionPool = void 0;
const axios_1 = __importDefault(require("axios"));
const perf_hooks_1 = require("perf_hooks");
const logger_1 = require("../../utils/logger");
class WhisperConnectionPool {
    poolSize;
    connections = [];
    ...
    constructor(poolSize = 5) {
        ...
        logger_1.logger.info('Whisper connection pool initialized', {
    ...
    async getConnection() {
        ...
        try {
            ...
            const acquisitionTime = perf_hooks_1.performance.now() - startTime;
            this.recordAcquisitionTime(acquisitionTime);
            // Log slow acquisition
            if (acquisitionTime > 50) {
                logger_1.logger.warn('Slow connection acquisition', {
            ...
            this.failedRequests++;
            logger_1.logger.error('Failed to acquire connection', {
    ...
    releaseConnection(connection) {
        if (!this.connections.includes(connection)) {
            logger_1.logger.warn('Attempting to release unknown connection');
            return;
        }
        ...
        logger_1.logger.debug('Connection released', {
            availableConnections: this.availableConnections.length,
            activeConnections: this.activeConnections.size,
        });
    }
    ...
    async checkAllConnectionsHealth() {
        logger_1.logger.info('Starting manual health check for all connections');
        const healthChecks = this.connections.map((conn) => this.checkConnectionHealth(conn));
        await Promise.allSettled(healthChecks);
        const stats = this.getStats();
        logger_1.logger.info('Manual health check completed', stats);
    }
    /**
     * Shut down the connection pool
     */
    async shutdown() {
        logger_1.logger.info('Shutting down Whisper connection pool');
        ...
        logger_1.logger.info('Whisper connection pool shut down');
    }
    initializePool() {
        ...
        logger_1.logger.debug('Connection pool initialized', {
            totalConnections: this.connections.length,
            availableConnections: this.availableConnections.length,
        });
    }
    createConnection() {
        const connection = axios_1.default.create({
        ...
        connection.interceptors.request.use((config) => {
            logger_1.logger.debug('Whisper API request', {
            ...
            logger_1.logger.error('Whisper API request error', { error: error.message });
        ...
        connection.interceptors.response.use((response) => {
            ...
            if (health) {
                ...
                if (health.failures >= 3) {
                    ...
                    logger_1.logger.warn('Connection marked as unhealthy', {
            ...
            logger_1.logger.error('Whisper API response error', {
    ...
    async acquireHealthyConnection() {
        ...
        while (attempts < maxAttempts) {
            ...
            logger_1.logger.warn('Replacing unhealthy connection');
    ...
    replaceConnection(oldConnection) {
    ...
    async performHealthCheck() {
        logger_1.logger.debug('Starting connection health check');
        ...
        try {
            ...
            if (health) {
                ...
                if (health.failures >= 2) {
                    health.healthy = false;
                    logger_1.logger.warn('Connection health check failed', {
                        failures: health.failures,
                        error: error instanceof Error ? error.message : String(error),
                    });
                }
    ...
    async warmupConnections() {
        ...
        const warmupPromises = connectionsToWarm.map(async (conn) => {
            try {
                ...
                logger_1.logger.debug('Connection warmed up successfully');
            }
            catch (error) {
                logger_1.logger.warn('Connection warmup failed', {
                    error: error instanceof Error ? error.message : String(error),
                });
            }
        });
...
exports.WhisperConnectionPool = WhisperConnectionPool;
//# sourceMappingURL=WhisperConnectionPool.js.map