...
exports.testProfilePictureService = testProfilePictureService;
const ProfilePictureService_1 = require("./files/upload/ProfilePictureService");
...
async function testProfilePictureService() {
    ...
    const service = new ProfilePictureService_1.ProfilePictureService();
    ...
    for (const testCase of testCases) {
        ...
        const result = service.validateProfilePicture(testCase.fileName, testCase.contentType, testCase.fileSize);
    ...
    const validator = new imageValidation_1.ImageValidator();
    // Test with a mock JPEG buffer (simplified)
    const mockJpegBuffer = Buffer.from([0xFF, 0xD8, 0xFF, 0xE0, ...Array(1000).fill(0)]);
    const validationResult = validator.validateImage('test.jpg', 'image/jpeg', mockJpegBuffer.length, mockJpegBuffer);
...
//# sourceMappingURL=test-profile-picture.js.map