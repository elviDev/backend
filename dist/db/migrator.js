...
exports.validateMigrations = exports.getMigrationStatus = exports.rollbackLastMigration = exports.runMigrations = void 0;
...
const initializeMigrationTable = async () => {
  ...
  `;
    await (0, database_1.query)(createTableSql);
...
const loadMigrationFiles = async () => {
    try {
        ...
        logger_1.logger.info({ count: migrations.length }, 'Loaded migration files');
        return migrations;
    }
    catch (error) {
        logger_1.logger.error({ error }, 'Failed to load migration files');
        throw new errors_1.DatabaseError('Failed to load migration files');
    }
};
/**
...
const getExecutedMigrations = async () => {
    const result = await (0, database_1.query)('SELECT * FROM migrations ORDER BY executed_at ASC');
    return result.rows;
};
/**
...
const executeMigration = async (migration) => {
    ...
    await (0, database_1.transaction)(async (client) => {
        logger_1.logger.info({ migration: migration.filename }, 'Executing migration');
        // Execute the migration SQL
        await client.query(migration.sql);
        // Record the migration execution
        const executionTime = Date.now() - startTime;
        const checksum = calculateChecksum(migration.sql);
        await client.query(`INSERT INTO migrations (id, filename, executed_at, checksum, execution_time_ms)
       VALUES ($1, $2, NOW(), $3, $4)`, [migration.id, migration.filename, checksum, executionTime]);
...
const runMigrations = async () => {
    try {
        ...
        if (pendingMigrations.length === 0) {
            logger_1.logger.info('No pending migrations found');
            return;
        }
        logger_1.logger.info({
        ...
        throw new errors_1.DatabaseError('Migration execution failed');
    }
};
...
const rollbackLastMigration = async () => {
    if (process.env.NODE_ENV === 'production') {
        throw new errors_1.DatabaseError('Rollback not allowed in production');
    }
    try {
        const executedMigrations = await getExecutedMigrations();
        ...
        const rollbackPath = (0, path_1.join)(process.cwd(), 'migrations', rollbackFilename);
        try {
            const rollbackSql = await (0, promises_1.readFile)(rollbackPath, 'utf-8');
            await (0, database_1.transaction)(async (client) => {
                logger_1.logger.warn({ migration: lastMigration.filename }, 'Rolling back migration');
            ...
            throw new errors_1.DatabaseError(`Rollback file not found or invalid: ${rollbackFilename}`, {
                lastMigration: lastMigration.filename,
            });
        }
    }
    catch (error) {
        logger_1.logger.error({ error }, 'Rollback failed');
        throw new errors_1.DatabaseError('Migration rollback failed');
    }
};
...
const getMigrationStatus = async () => {
    try {
        ...
        logger_1.logger.error({ error }, 'Failed to get migration status');
        throw new errors_1.DatabaseError('Failed to get migration status');
    }
};
