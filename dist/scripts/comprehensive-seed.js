...
class ComprehensiveSeeder {
    ...
    async run() {
        try {
            ...
            logger_1.logger.info('✅ Comprehensive seeding completed successfully!');
        }
        catch (error) {
     ...
     */
    async clearAllData() {
        logger_1.logger.info('🧹 Clearing all existing data...');
        const tables = [
        ...
        for (const table of tables) {
            try {
                ...
                await (0, database_1.query)(`TRUNCATE TABLE ${table} CASCADE`);
                logger_1.logger.info(`  ✓ Cleared ${table}`);
            }
            catch (error) {
                // If TRUNCATE fails, try DELETE as fallback
                try {
                    await (0, database_1.query)(`DELETE FROM ${table}`);
                    logger_1.logger.info(`  ✓ Cleared ${table} (fallback)`);
                }
                catch (deleteError) {
                    logger_1.logger.warn(`  ⚠️  Could not clear ${table}:`, error.message);
                }
            }
        }
        // Reset sequences (if they exist - most tables use UUIDs)
        try {
            await (0, database_1.query)(`SELECT setval(pg_get_serial_sequence('categories', 'id'), 1, false)`);
        }
        catch (error) {
            // Sequence might not exist, continue
        }
        logger_1.logger.info('✅ Data clearing completed');
    }
    /**
     * Create 5 test users: 1 CEO, 2 Managers, 2 Staff
     */
    async createUsers() {
        logger_1.logger.info('👥 Creating test users...');
        const userData = [
            { name: 'Alex Johnson', email: 'alex.ceo@company.com', role: 'ceo' },
            { name: 'Sarah Manager', email: 'sarah.manager@company.com', role: 'manager' },
            { name: 'Mike Manager', email: 'mike.manager@company.com', role: 'manager' },
            { name: 'Emma Staff', email: 'emma.staff@company.com', role: 'staff' },
            { name: 'John Staff', email: 'john.staff@company.com', role: 'staff' },
        ];
      ...
      `, [
            ...
            logger_1.logger.info(`  ✓ Created ${user.role}: ${user.name} (${user.email})`);
        }
        logger_1.logger.info(`✅ Created ${this.users.length} users`);
    }
    /**
     * Create 5 channels in different categories
     */
    async createChannels() {
        logger_1.logger.info('📂 Creating test channels...');
      ...
      `, [channelData.members.length, channelId]);
            ...
            logger_1.logger.info(`  ✓ Created channel: ${channelData.name} (${channelData.channel_type})`);
        }
        logger_1.logger.info(`✅ Created ${this.channels.length} channels`);
    }
    /**
     ...
     */
    async createMessagesAndThreads() {
        logger_1.logger.info('💬 Creating messages, threads, and replies...');
        ...
        for (const channel of this.channels) {
            logger_1.logger.info(`  Creating content for channel: ${channel.name}`);
        ...
        logger_1.logger.info(`✅ Created messages and threads for all channels`);
    }
    /**
     * Create 3 tasks per channel with 5 comments each
     */
    async createTasksAndComments() {
        logger_1.logger.info('📋 Creating tasks and comments...');
        const taskTitles = [
        ...
        for (const channel of this.channels) {
            logger_1.logger.info(`  Creating tasks for channel: ${channel.name}`);
        ...
        logger_1.logger.info(`✅ Created ${this.tasks.length} tasks with comments`);
    }
    /**
     * Add reactions to messages
     */
    async createReactions() {
        logger_1.logger.info('👍 Adding reactions to messages...');
        const emojis = ['👍', '❤️', '😊', '🎉', '👏', '🔥', '💯', '✅'];
        let reactionCount = 0;
        ...
        logger_1.logger.info(`✅ Added ${reactionCount} reactions`);
    }
    /**
     * Generate notifications and activities for all users (minimum 5 each)
     */
    async createNotificationsAndActivities() {
        logger_1.logger.info('🔔 Creating notifications and activities...');
        ...
        logger_1.logger.info(`✅ Created notifications and activities for all users`);
    }
    ...
    getActivityDescription(type, userName) {
        const descriptions = {
    ...
    async generateSummaryReport() {
        logger_1.logger.info('📊 Generating summary report...');
        const stats = {
            users: (await (0, database_1.query)('SELECT COUNT(*) as count FROM users')).rows[0].count,
            channels: (await (0, database_1.query)('SELECT COUNT(*) as count FROM channels')).rows[0].count,
            messages: (await (0, database_1.query)('SELECT COUNT(*) as count FROM messages')).rows[0].count,
            tasks: (await (0, database_1.query)('SELECT COUNT(*) as count FROM tasks')).rows[0].count,
            comments: (await (0, database_1.query)('SELECT COUNT(*) as count FROM task_comments')).rows[0].count,
            reactions: (await (0, database_1.query)('SELECT COUNT(*) as count FROM message_reactions')).rows[0].count,
            notifications: (await (0, database_1.query)('SELECT COUNT(*) as count FROM notifications')).rows[0].count,
            activities: (await (0, database_1.query)('SELECT COUNT(*) as count FROM activities')).rows[0].count,
            threads: (await (0, database_1.query)('SELECT COUNT(*) as count FROM messages WHERE thread_root_id IS NOT NULL')).rows[0].count,
            directReplies: (await (0, database_1.query)('SELECT COUNT(*) as count FROM messages WHERE reply_to_id IS NOT NULL AND thread_root_id IS NULL')).rows[0].count
        };
        logger_1.logger.info('\n📋 SEEDING SUMMARY REPORT');
        logger_1.logger.info('========================');
        logger_1.logger.info(`👥 Users: ${stats.users} (1 CEO, 2 Managers, 2 Staff)`);
        logger_1.logger.info(`📂 Channels: ${stats.channels} (different categories)`);
        logger_1.logger.info(`💬 Messages: ${stats.messages} total`);
        logger_1.logger.info(`🧵 Thread Replies: ${stats.threads}`);
        logger_1.logger.info(`↩️  Direct Replies: ${stats.directReplies}`);
        logger_1.logger.info(`📋 Tasks: ${stats.tasks} (3 per channel)`);
        logger_1.logger.info(`💭 Comments: ${stats.comments} (5 per task)`);
        logger_1.logger.info(`👍 Reactions: ${stats.reactions}`);
        logger_1.logger.info(`🔔 Notifications: ${stats.notifications} (5+ per user)`);
        logger_1.logger.info(`📈 Activities: ${stats.activities} (5+ per user)`);
        logger_1.logger.info('========================\n');
        // User details
        logger_1.logger.info('👥 USER ACCOUNTS:');
        for (const user of this.users) {
            logger_1.logger.info(`  ${user.role.toUpperCase()}: ${user.name} (${user.email}) - Password: ${user.password}`);
        }
        // Channel details  
        logger_1.logger.info('\n📂 CHANNELS:');
        for (const channel of this.channels) {
            logger_1.logger.info(`  ${channel.name} (${channel.channel_type}) - ${channel.privacy_level}`);
        }
...
exports.default = ComprehensiveSeeder;
