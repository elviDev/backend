...
exports.addTaskComments = addTaskComments;
...
async function addTaskComments() {
    try {
        logger_1.logger.info('🔄 Adding comments and reactions to tasks...');
        // Get all tasks
        ...
        logger_1.logger.info(`📊 Found ${tasks.length} tasks and ${users.length} users`);
        let commentsAdded = 0;
        let reactionsAdded = 0;
        for (const task of tasks.slice(0, 20)) { // Add comments to first 20 tasks
            try {
                logger_1.logger.info(`💬 Adding comments to task: ${task.title}`);
                ...
                for (let i = 0; i < numComments; i++) {
                    ...
                    commentsAdded++;
                    logger_1.logger.debug(`  ✅ Comment added by ${commenter.name}: "${commentText.substring(0, 40)}..."`);
                    ...
                    if (Math.random() > 0.5 && allowedCommenters.length > 1) {
                        ...
                        if (replier) {
                            ...
                            commentsAdded++;
                            logger_1.logger.debug(`    ↳ Reply by ${replier.name}: "${replyText.substring(0, 30)}..."`);
                        }
                    }
                    ...
                    if (Math.random() > 0.7) {
                        try {
                            ...
                            if (reactor && reactor.id !== commenter.id) {
                                const reactions = ['up', 'down', 'thumbs_up', 'thumbs_down'];
                            ...
                            logger_1.logger.warn(`Failed to add reaction: ${reactionError}`);
                        }
                    }
                }
                logger_1.logger.info(`  ✅ Added ${numComments} comments to "${task.title}"`);
            }
            catch (error) {
        ...
        logger_1.logger.info(`  - Tasks with comments: ${Math.min(20, tasks.length)}`);
        return {
            commentsAdded,
            reactionsAdded,
        ...
        logger_1.logger.error('❌ Failed to add task comments:', error);
        throw error;
    }
}
...
//# sourceMappingURL=add-task-comments.js.map